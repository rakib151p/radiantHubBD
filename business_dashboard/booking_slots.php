                ...
                <main class="flex-1 overflow-x-hidden overflow-y-auto bg-gray-200">
                    <div class="container">
                        ...
                        while ($row_worker = $result_worker->fetch_assoc()) {
                            ...
                            $sql_schedule = "SELECT * FROM bookings WHERE date='$date' AND worker_id='$worker_id' AND status !='cancelled' order by booking_time";
                            ...
                            while ($row_schedule = $result_schedule->fetch_assoc()) {
                                ...
                                $date = $row_schedule['date'];
                                $time = $row_schedule['booking_time'];
                                ...
                                $is_completed_enabled = $booking_datetime < $current_datetime && ($row_schedule['shop_end'] !== 1) ? 'enabled' : 'disabled';
                                // echo $is_completed_enabled;
                                ...
                                echo '<div class="time-slot">
                                    ...
                                    <p>&nbsp;</p>';
                                if ($row_schedule['status'] == 'completed') {
                                    echo '<button class="book-button" disabled>Completed</button>';
                                } else if ($row_schedule['shop_end']) {
                                    echo '<button class="book-button" disabled>Submitted</button>';
                                } else {
                                    echo '<button class="book-button" data-booking-id="' . $booking_id . '" ' . ($is_completed_enabled == 'enabled' ? '' : 'disabled') . '>Confirm?</button>';
                                }

                                echo '<button class="report-button" data-booking-id="' . $booking_id . '">Report</button>
